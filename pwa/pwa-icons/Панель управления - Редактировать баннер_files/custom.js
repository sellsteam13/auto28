...
$(document).ready(function(){
    ...
    $("#adminRegion").change(function(){
        ...
        $.ajax({
                url: '/workzone/getregion/' + $(this).val(),
                type:'get',
                dataType: 'html',
    ...
    $("#add-format").click(function(){
        var prototype = $(this).next("#category_formats").data("prototype");
        var count = $(this).parent().find(".table-formats > tbody > tr").length;
        var newForm = prototype.replace(/__name__/g, count);
        $(this).parent().find(".table-formats > tbody").append(newForm);
    });
    
    /*
    setInterval(function(){
        $.ajax({
            url: '/workzone/getheaderalerts',
            type:'get',
            dataType: 'html',
            success: function(data)
            {
                $("#headerBellAlerts > li.dropdown.ajax").html(data);
            },
            error: function(xhr, ajaxOptions, thrownError) {
                err=xhr.responseText;
            }
...
function complaintAnswer(complaintId){
    $.ajax({
        url: '/workzone/complaintanswer/' + complaintId,
        type:'get',
        dataType: 'html',
        success: function(data){
            $('textarea[name="complaintAnswer"]').val(data);
            $('input[name="complaint"]').val(complaintId);
            $("#complaintAnswerModal").modal();
        },
        error: function(xhr, ajaxOptions, thrownError) {
            err=xhr.responseText;
        }
...
function answerToComplaint(element){
    var complaintId = $("#complaintAnswerModal").find("input[type='hidden']").val();
    
    var fd = new FormData;
    $("#complaintAnswerModal").find('input[type="hidden"]').each(function(){
        fd.append($(this).attr('name'), $(this).val());
    });
    $("#complaintAnswerModal").find('textarea[name="complaintAnswer"]').each(function(){
        fd.append($(this).attr('name'), $(this).val());
    });
    $.ajax({
        url:'/workzone/answercomplaint',
        data: fd,
        processData: false,
        contentType: false,
        method: 'post',
        success : function(html){
            $("#complaintItem" + complaintId).removeClass("active").html(html);
            $("#complaintAnswerModal").modal('hide');
        }
    });
}
...
function getChildCategories(categoryId, element, spaces)
{
    ...
    {
        $.ajax({
            url: '/workzone/getsub/category/' + categoryId + '/' + spaces,
            type:'get',
            dataType: 'html',
            beforeSend: function(){element.append('<i class="fa fa-spinner fa-spin"></i>')},
            success: function(html)
...
function changeComplaintStatus(element, complaintId)
{
    ...
    $.ajax({
            url: '/workzone/changecomplaintstatus/' + complaintId + '/' + complaintStatus,
            type:'get',
            dataType: 'html',
            beforeSend: function(){
